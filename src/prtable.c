...
static struct itable *sort_list(struct ilist *ilist) {
  ...
  for (n = 0, p = ilist->head; p != NULL && n < ilist->nelems; p = p->next, n++) {
    itable->table[n] = p;
  }
  itable->nelems = n;
...
static void gen_inst(struct ilist *ilist, const struct target *t, unsigned char undoc, const char *p, size_t bufi, const char *pr) {
  size_t      bufk;
  const char *s;
...
static struct ilist *gen_list(const struct target *t, unsigned char mask2, int delta) {
  int                    i, pr;
  ...
  while (mt[i].pat != NULL) {
    ...
    if (t->mask == 1 && (mt[i].mask & 1)) {
      ...
      if ((mt[i].mask & t->mask) && !(mt[i].mask & mask2)) {
        pr = 1;
      }
    ...
    if (pr) {
      gen_inst(ilist, t, mt[i].undoc, mt[i].pat, 0, mt[i].pr);
    }
    i++;
  }
