  ...
  if (i >= 0) {
    ...
    default:
      if (s_target->genf(&b, *p, vs, i, savepc) == -1) {
        eprogname();
        fprintf(stderr, _("fatal:  bad pattern %s ('%c')"), p_orig, *p);
        enl();
        exit(EXIT_FAILURE);
      }
    }
...
static int isoctal(int c) { return c >= '0' && c <= '7'; }
...
enum strmode { STRMODE_ECHO, STRMODE_NULL, STRMODE_BYTE, STRMODE_WORD };
...
static const char *d_byte(const char *p) { return d_lst(p, 0); }

static const char *d_word(const char *p) { return d_lst(p, 1); }
...
loop:
  if (*cp == '\0' || *cp == ';') {
    ...
    if (s_pass == 1) {
      list_endln();
      list_startln(s_empty_line, curfile()->linenum, s_pc, nfiles());
    }
    cp++;
    goto start;
  } else if (*cp == '.') {
    ...
    if (col0 && *cp != '.') {
      ...
      if (s_pass == 1 && !s_lastsym->isequ && s_lastsym->val != s_pc) {
        eprint(_("misaligned label %s\n"), s_lastsym->name);
        fprintf(stderr,
                _(" Previous value was %XH, "
                  "new value %XH."),
                s_lastsym->val, s_pc);
        eprcol(s_pline, s_pline_ep);
        newerr();
...
static void dopass(const char *fname) {
  ...
  while (nfiles() > 0) {
    ...
    if (getlin(curfile()->fin) >= 0) {
      if (s_pass == 1) {
        list_startln(s_line, curfile()->linenum, s_pc, nfiles());
      }
      pp_line(s_line);
      if (s_pass == 1)
        list_skip(s_skipon);
